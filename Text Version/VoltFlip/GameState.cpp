    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            if (board[i][j].points == Voltorb) {
                ...
                row[i].points_sum += board[i][j].points;
                col[j].points_sum += board[i][j].points;
            }
        }
    }
}
...
void GameState::disp_board(void)
{
    ...
    for (int i = 0; i < height; i++) {
        ...
        cout << setw(2) << row[i].points_sum << '/' << row[i].voltorbs_sum << endl;
        cout << lower_border;
    }
    cout << '|';
    for (int j = 0; j < width; j++) {
        cout << setw(2) << col[j].points_sum << '/' << col[j].voltorbs_sum << '|';
    }
    cout << endl;
}
